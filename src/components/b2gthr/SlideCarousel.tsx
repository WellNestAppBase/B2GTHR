...
const SlideCarousel: React.FC<SlideCarouselProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        ...
        // Clear key pressed state after animation
  ...
  }, [pageTitles.length, initialIndex, currentPage]);
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-black/20">
      ...
      <button
        ...
        className={`fixed left-3 top-1/2 -translate-y-1/2 z-30 bg-black/30 hover:bg-black/50 text-white p-1.5 rounded-full transition-all ${currentPage === 0 ? "opacity-30 cursor-not-allowed" : "opacity-100"} ${keyPressed === "ArrowLeft" ? "scale-125 bg-white/20" : ""}`}
      ...
      <button
        ...
        className={`fixed right-3 top-1/2 -translate-y-1/2 z-30 bg-black/30 hover:bg-black/50 text-white p-1.5 rounded-full transition-all ${currentPage === pageTitles.length - 1 ? "opacity-30 cursor-not-allowed" : "opacity-100"} ${keyPressed === "ArrowRight" ? "scale-125 bg-white/20" : ""}`}
