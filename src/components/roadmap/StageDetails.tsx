...
interface StageDetailsProps {
  ...
  gamePosition: { get: () => number }; // Simple getter function
...
const StageDetails: React.FC<StageDetailsProps> = ({
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel({
    ...
    slidesToScroll: 1,
    // Note: draggable is handled by watchDrag below
  ...
  // Disable dragging and other automatic behaviors
  useEffect(() => {
    if (emblaApi) {
      // Use the reInit method with only the supported options
      emblaApi.reInit({
        loop: false,
        align: "center",
        dragFree: false,
      ...
      // Manually disable dragging using the appropriate API methods
      const disableDrag = () => false;
      emblaApi.on("pointerDown", disableDrag);

      // Clean up event listener when component unmounts
      return () => {
        if (emblaApi) {
          // Add this check to fix the type error
          emblaApi.off("pointerDown", disableDrag);
        }
      };
    }
  }, [emblaApi]);
